...
def _grouped_conv_block(inputs, grouped_channels, kernel_size=1, strides=1, weighted_decay=5e-4, cardinality=32):
    ...
    for i in range(cardinality):
        x = Conv3D(grouped_channels, kernel_size, strides, padding='same', use_bias=False, kernel_initializer='he_normal',
                   kernel_regularizer=l2(weighted_decay))(inputs)
        x = Conv3D(grouped_channels, kernel_size * 3, strides, padding='same', use_bias=False, kernel_initializer='he_normal',
...
def _bottleneck_block(x, filters=64, kernel_size=1, strides=1, weighted_decay=5e-4, cardinality=8):
    ...
    if K.image_data_format() == 'channels_first':
        if init._keras_shape[1] != 2 * filters:
            init = Conv3D(filters * 2, kernel_size, strides, padding='same', use_bias=False, kernel_initializer='he_normal',
